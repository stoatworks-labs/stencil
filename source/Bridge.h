...
constexpr uint16_t kNone = 65535;
...
/**
	...
	cell the coordinates of the nearest sheet cell whose label differs from
	that cell's own nearest -- for a sheet cell, the nearest cell of another
	piece -- as two values a cell (x, y), or kNone. Returns false on failure.
*/
using Flood = std::function< bool( const std::vector< uint32_t >& labels, int width, int height,
